...
SQL
      ...
      db[query].each do |post|
        ...
        wordpress_url = "http://www.drtoast.com/crumbs/#{post[:ID]}"
        ...
        data = {
           ...
           'date' => date,
           'categories' => post[:categories].split(/,/).reject{|a| ['elsewhere'].include?(a)}
        ...
        htaccess << "Redirect 301 %s http://www.drtoast.com/%02d/%02d/%02d/%s.html\n" % [wordpress_url, date.year, date.month, date.day, slug]
      end
    end
  end
end

Jekyll::WordPress.process("toastwp", "root", "", "127.0.0.1")
